...
export const BRAND_SCENE_MANDATES: Record<string, string> = {
  Roosterbet:  'FIRE IS MANDATORY AND MUST ORIGINATE FROM THE MAIN SUBJECT: Keep the EXACT hero subject described in the base scene as a solid, real, fully-formed photorealistic figure or object — its identity, true surface, material and details sharp, unmistakable and dominant in the frame — and add flames rising from its body/surface as a glowing fiery aura, with warm red/crimson/orange light in the surroundings. Mention the fire only once and keep the subject the clear focus. This is the Roosterbet signature.',
