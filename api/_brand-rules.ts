...
export const BRAND_SCENE_MANDATES: Record<string, string> = {
  Roosterbet:  'FIRE IS MANDATORY AND MUST WRAP THE SUBJECT AS ONE UNIFIED FLAME: The athlete (and the ball, if present) MUST be engulfed by a single continuous burning aura — fire embraces their silhouette like an outer layer of their body, as if the subject IS the source of the flame. The fire must look like ONE explosion or ONE burning halo wrapping around them, NOT separate jets, NOT multiple streams, NOT individual fire trails from feet/arms/movement. Picture a character on fire from within: the flame conforms to their shape, swallows the ball, and reads as a single unified mass of fire. Embers and heat shimmer may radiate outward from that single mass, but the core fire body is ONE continuous flame wrapping the subject. This is the Roosterbet signature.',
