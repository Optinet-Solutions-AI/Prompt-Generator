...
export const BRAND_SCENE_MANDATES: Record<string, string> = {
  Roosterbet:  'FIRE IS MANDATORY AND MUST COME FROM THE PLAYER AND THE BALL: The athlete is the SOURCE of the fire — flames emerge directly from the player\'s body and engulf them, AND the ball must also be on fire as part of the SAME continuous burning mass. The fire is generated BY the subject (player + ball), it does NOT live in the stadium, floor, crowd, or background. This burning mass MUST read as ONE unified wrapping flame around the player and ball — like the player is burning from within and the ball is on fire as part of the same explosion — NOT as multiple separate jets shooting from limbs, NOT as scattered streams from feet/arms/movement trail, NOT as a background-only effect detached from the player. Picture: a player wrapped in a single body of flame, holding a ball that is also wrapped in the same flame, with the fire flowing as ONE continuous connected mass around them. The flame conforms to the player\'s silhouette and embraces the ball. Embers and heat shimmer may radiate outward, but the core fire is ONE continuous flame attached to the player and ball. This is the Roosterbet signature.',
