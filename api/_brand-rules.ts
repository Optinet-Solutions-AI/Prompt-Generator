...
export const BRAND_SCENE_MANDATES: Record<string, string> = {
  Roosterbet:  'FIRE IS MANDATORY AND MUST ORIGINATE FROM THE PLAYER AND THE BALL: Render a REAL, photorealistic human athlete — a clearly visible person with natural skin, a real face, and a team kit — with intense flames erupting FROM the player\'s body and trailing off them (rising and streaming off the shoulders, back, arms, and legs), and the ball also ablaze as part of the same fiery energy. The fire comes OUT OF the player like a blazing aura wrapping around them, but the athlete\'s body, face, and uniform MUST stay fully visible, solid, and realistic. The player is NOT made of fire, NOT a flaming skeleton, NOT a silhouette of flame, and is NOT replaced or fully engulfed by fire — the flames are an EFFECT radiating from a real person (a real player wreathed in fire), not the material the player is built from. Do NOT confine the fire to the background, stadium, floor, or crowd; it must originate from and cling to the player and the ball. Picture a real footballer charging forward, flames pouring off their body and a burning ball at their feet. This is the Roosterbet signature.',
