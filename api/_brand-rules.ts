...
export const BRAND_SCENE_MANDATES: Record<string, string> = {
  Roosterbet:  'FIRE IS MANDATORY AND MUST ORIGINATE FROM THE MAIN SUBJECT: Take the EXACT hero subject described in the base scene and give it intense flames rising and erupting from it as the Roosterbet signature. Render that subject as a solid, real, fully-formed photorealistic figure or object, with its true surface, material and detail clearly visible, wrapped in living flames that radiate off its surface like a blazing fiery aura. The flames flow off the subject while the surroundings glow with fiery red, crimson and orange energy. This is the Roosterbet signature.',
