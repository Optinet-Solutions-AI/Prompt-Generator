  ...
  try {
    ...
    let wideBannerRule = '';
    {
      ...
      if (!isNaN(ratio) && ratio >= 1.7) {
        wideBannerRule = `\n8) WIDE BANNER COMPOSITION (OVERRIDES THE BASE PROMPT'S FRAMING)\nThis is a WIDE horizontal banner (aspect ${effectiveAspect}). It MUST be framed as a wide ESTABLISHING shot, NOT a close-up.\nIMPORTANT: If the Base prompt specifies a close-up, portrait, head-and-shoulders, "sharp focus on face", or any tight/shallow framing, DELETE that wording and REPLACE it with this wide establishing shot — the wide-banner framing takes priority over the base prompt. Keep the subject's identity, wardrobe and style intact.\nSUBJECT SIZE — FIT THE WHOLE SUBJECT WITH MARGIN: Pull the camera back so the subject is sized SMALL-TO-MEDIUM in the frame, occupying roughly the central 55–70% of the height. The ENTIRE subject must be visible and fully contained — including every extremity and any equipment that extends from it (the top of the head / comb / hat / helmet, raised arms or hands, a ball, a racket, a basketball rim or backboard above the player, and the feet or base below). Use a dynamic, natural, off-centre pose — do NOT make it a stiff, symmetric, dead-centre portrait.\nCROP-SAFE MARGINS (CRITICAL): This banner is CROPPED on the top and bottom edges — roughly the top 12% and bottom 12% of the frame are trimmed away. Therefore leave a CLEAR MARGIN of plain continuous background ABOVE the highest point of the subject/equipment and BELOW the lowest point (the feet/base). Nothing important — no head, no raised hand, no rim, no backboard, no feet, no logo, no text — may sit inside those top/bottom bands; only background scenery may be there, so the crop removes only background and never clips the subject.\nFULL-BLEED BACKGROUND: The environment (arena, crowd, sky, walls, floor, stage) must fill the ENTIRE frame edge to edge with continuous scenery — NO black bars, NO letterbox, NO borders, NO empty/white/solid-coloured strip anywhere, including the trimmed top and bottom bands.\n`;
      }
    }
