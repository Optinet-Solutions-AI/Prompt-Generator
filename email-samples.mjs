...
// Trims transparent padding so the wordmark fills its bounds (lets the
// email-layer dark pill, if any, wrap tightly instead of around empty space).
async function wordmarkDataUri(slug) {
  ...
  try {
    await fs.access(p);
    // Render large, then trim transparent edges so the PNG's bounding box
    // matches the visible content.
    const rendered = await sharp(p, { density: 256 }).png().toBuffer();
    let trimmed;
