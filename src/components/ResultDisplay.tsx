...
export function ResultDisplay({
  ...
  // All of the dialog's own state (mode, title, paste/dissect fields, etc.)
  // now lives inside SaveAsReferenceDialog itself — this page only tracks
  // whether it's open.
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Save as New Reference dialog — triggered by the 💾 toolbar button.
          ...
          fields, which are then editable before saving). `generated` is
          built from the metadata already on screen, so "From this prompt"
          stays available and is the default mode here. */}
                    ...
                    {images.map((img, index) => {
                      ...
                      return (
                        <div
                          ...
                          style={{ aspectRatio: imageAspects[imageId] || ratioToCss(metadata?.aspectRatio) }}
                          onClick={() => {
