...
import {
  newBlock, moveBlock, removeBlock, updateBlock, defaultEmailDoc, EMAIL_SAFE_FONTS,
  ...
  const renderStyle = (b: EmailBlock) => {
    ...
    return (
      <div className="mt-2 pt-2 border-t border-border grid grid-cols-2 gap-2">
        ...
        {b.type === 'header' && b.mode !== 'text' && <div><Label className="text-[10px] mb-0.5 block">Logo padding (px)</Label><Input type="number" value={num(st.logoPad)} onChange={e => patchStyle(b.id, 'logoPad', e.target.value ? Number(e.target.value) : undefined)} placeholder="12" className="h-7 text-xs" /></div>}
